...
def get_font(font_face, font_size):
    '''Gets a font object'''
    if font_face not in FONT_FACE_DICT:
        raise 'Not a valid font face: '
    if (font_face,font_size) not in FONT_DICT:
...
class Frame(object):
    ...
    開始=         start
    停止=         stop
    設畫處理=     set_draw_handler
    設背景色=     set_background_color
    設鼠左擊處理= set_mouse_left_click_handler
    設鼠右擊處理= set_mouse_right_click_handler
    設鼠動處理=   set_mouse_move_handler
    設鍵下處理=   set_key_down_handler
    設鍵上處理=   set_key_up_handler
    加鈕=         add_button
    加標=         add_label
    控擊處理=     control_click_handler
    叫畫處理=     call_draw_handler
    跑=           run
...
if __name__ == '__main__':
    ...
    框.設鍵下處理(  lambda x: 印事件('鍵下:',   x))
    框.設鍵上處理(  lambda x: 印事件('鍵上:',   x))
    框.設鼠左擊處理(lambda x: 印事件('鼠左擊:', x))
    框.設鼠右擊處理(lambda x: 印事件('鼠右擊:', x))
    框.設鼠動處理(  lambda x: 印事件('鼠動:',   x))
    框.設畫處理(畫)

    框.開始()#nothing can come after frame.start()
